...
export class ApplicationLoadBalancerEcsEc2Stack extends cdk.Stack {
  ...
  constructor(scope: cdk.Construct, id: string, props: EcsApplicationLoadBalancerStackProps) {
    ...
    this.createWebContainer(scope, "express-react-nextjs-frontend", props.vpc, {
      ...
      logging: new AwsLogDriver({ streamPrefix: "express-react-nextjs-frontend" }),
      portMappings: [
        { containerPort: 3000, hostPort: 80, protocol: ecs.Protocol.TCP }
      ]
    });

    this.createWebContainer(scope, "express-react-nextjs-backend", props.vpc, {
      ...
      logging: new AwsLogDriver({ streamPrefix: "express-react-nextjs-backend" }),
      portMappings: [
